...
@cli.command()
@click.option('--local-dir', type=str, default='online-materialdesign-final')
...
def materialdesign(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    tune.run(online, config={
        ...
        "task_kwargs": {'seed': tune.randint(1000),
                        'split_percentile': tune.grid_search([20.0, 50.0, 90.0])},
        "is_discrete": True,
        ...
        "solver_warmup": 50,
        "solver_steps": 1,
        "solver_beta": tune.grid_search([0.0, 0.1, 0.3, 0.5, 0.7, 0.9])},
...
def molecule(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    tune.run(online, config={
        ...
        "lookahead_steps": 1,
        "lookahead_backprop": True,
        "evaluate_steps": [i for i in range(0, 2000, 10)],
        "solver_lr": 0.01,
        "solver_interval": 1,
        "solver_warmup": 50,
        "solver_steps": 1,
