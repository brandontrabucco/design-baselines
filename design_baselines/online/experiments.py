...
def hopper_particle(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    tune.run(online, config={
        ...
        "forward_model_lr": 0.001,
        "initial_alpha": 1.0,
        "initial_beta": tune.grid_search([0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.1, 1.3, 1.5, 2.0]),
        "alpha_lr": 0.05,
        "target_conservatism": 0.05,
...
@cli.command()
@click.option('--local-dir', type=str, default='online-hopper-playground')
@click.option('--cpus', type=int, default=24)
@click.option('--gpus', type=int, default=1)
@click.option('--num-parallel', type=int, default=1)
@click.option('--num-samples', type=int, default=1)
def hopper_playground(local_dir, cpus, gpus, num_parallel, num_samples):
    """Evaluate Conservative Score Models on HopperController-v0
    """
