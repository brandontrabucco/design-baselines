...
def online_hopper(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    from design_baselines.online_v2 import online_ensemble
    ray.init(num_cpus=cpus,
             num_gpus=gpus,
             temp_dir=os.path.expanduser('~/tmp'))
    tune.run(online_ensemble, config={
        "logging_dir": "data",
...
def conservative_ensemble_dkitty(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    tune.run(conservative_ensemble, config={
        ...
        "initial_alpha": tune.grid_search([.0, .00005, .0001, .0002, .0005, 0.001, 0.002, 0.004]),
        "alpha_lr": .0,
        "perturbation_lr": 0.00002,
        "perturbation_steps": tune.grid_search([50]),
...
@cli.command()
@click.option('--local-dir', type=str, default='perturbation-backprop-dkitty')
@click.option('--cpus', type=int, default=24)
@click.option('--gpus', type=int, default=1)
@click.option('--num-parallel', type=int, default=1)
@click.option('--num-samples', type=int, default=1)
def perturbation_backprop_dkitty(local_dir, cpus, gpus, num_parallel, num_samples):
    """Train a forward model using various regularization methods and
    solve a model-based optimization problem
    ...
    tune.run(conservative_ensemble, config={
        ...
        "perturbation_steps": tune.grid_search([0, 6, 12, 18, 25]),
...
def conservative_ensemble_ant(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    tune.run(conservative_ensemble, config={
        ...
        "forward_model_lr": 0.001,
        "target_conservative_gap": 0.0,
        "initial_alpha": tune.grid_search([2.0, 1.0, 0.5, 0.25, 0.1]),
        "alpha_lr": 0.0,
        "perturbation_lr": tune.grid_search([0.002, 0.001, 0.0005, 0.0002, 0.0001, 0.00005]),
...
@cli.command()
@click.option('--local-dir', type=str, default='perturbation-backprop-ant')
@click.option('--cpus', type=int, default=24)
@click.option('--gpus', type=int, default=1)
@click.option('--num-parallel', type=int, default=1)
@click.option('--num-samples', type=int, default=1)
def perturbation_backprop_ant(local_dir, cpus, gpus, num_parallel, num_samples):
    """Train a forward model using various regularization methods and
    solve a model-based optimization problem
...
def perturbation_backprop_gfp(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    tune.run(conservative_ensemble, config={
        ...
        "perturbation_backprop": True,
        "perturbation_lr": 18.0,
        "perturbation_steps": tune.grid_search([0, 12, 25, 37, 50]),
        "solver_lr": tune.sample_from(lambda c: c['config']['perturbation_lr']),
        "solver_samples": 128,
        "solver_steps": 500,
        "target_conservative_gap": 0.0,
...
@cli.command()
@click.option('--local-dir', type=str, default='perturbation-backprop-superconductor')
@click.option('--cpus', type=int, default=24)
@click.option('--gpus', type=int, default=1)
@click.option('--num-parallel', type=int, default=1)
@click.option('--num-samples', type=int, default=1)
def perturbation_backprop_superconductor(local_dir, cpus, gpus, num_parallel, num_samples):
    """Train a forward model using various regularization methods and
    solve a model-based optimization problem
...
def mins_ant(local_dir, cpus, gpus, num_parallel, num_samples):
    ...
    tune.run(model_inversion, config={
        ...
        "epochs_per_iteration": 20,
        "iterations": 100,
        "exploration_samples": 128,
        "exploration_rate": 20.,
        "thompson_samples": 128,
        "solver_samples": 128},
        num_samples=num_samples,
        local_dir=local_dir,
        resources_per_trial={'cpu': cpus // num_parallel,
