...
from design_baselines.vary_architecture.trainers import Ensemble
from design_baselines.vary_architecture.nets import ForwardModel
...
def vary_architecture(config):
    ...
    for i in range(1, config['solver_steps'] + 1):
        ...
        # save the gradients and norms of every model
        grads = []
        norms = []

        # back propagate through each forward model
        for fm in forward_models:
            ...
            # evaluate the gradient and the norm
            grad = tape.gradient(score, x)
            norm = tf.linalg.norm(tf.reshape(
                grad, [-1, task.input_size]), axis=-1)
            while len(norm.shape) < len(grad.shape):
                norm = norm[..., tf.newaxis]

            # record these for safe keeping
            grads.append(grad)
            norms.append(norm)

        # calculate the slowest moving gradient in the ensemble
        minimum_norm = tf.reduce_min(norms, axis=0)
        grads = [g * (minimum_norm / n) for g, n in zip(grads, norms)]
