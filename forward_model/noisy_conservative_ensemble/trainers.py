...
class NoisyConservativeEnsemble(tf.Module):
    ...
    def optimize(self,
        ...
        for step in range(self.perturbation_steps):
            with tf.GradientTape() as tape:
                tape.watch(x)
                score = fm.get_distribution(x, **kwargs).sample()
    ...
    def train_step(self,
        ...
        X: tf.Tensor
            a batch of training inputs shaped like [batch_size, channels]
        original_y: tf.Tensor
            a batch of training labels shaped like [batch_size, 1]
        b: tf.Tensor
            bootstrap indicators shaped like [batch_size, num_oracles]
        z: tf.Tensor
            bootstrap label noise shaped like [batch_size, num_oracles]
        ...
        for i in range(self.bootstraps):
            ...
            # add label noise to the data set to increase diversity
            y = original_y + z[:, i:i + 1]
    ...
    def train(self,
        ...
        for X, y, b, z in dataset:
            for name, tensor in self.train_step(X, y, b, z).items():
