...
def one_hot_encode_aa(aa_str, pad=None):
    """
    Code adapted from https://github.com/dhbrookes/CbAS/blob/master/util.py
    """
    M = len(aa_str)
...
def get_experimental_X_y(random_state=1,
    ...
    basedir = os.path.dirname(os.path.abspath(__file__))
    df = pd.read_csv(os.path.join(basedir, 'gfp_data.csv'))
    X, _ = get_gfp_X_y_aa(df, large_only=True, ignore_stops=True)
    y_gt = np.load(os.path.join(basedir, "gfp_gt_evals.npy"))
    if return_test:
...
class ProteinFluorescenceDataset(object):
    ...
    def load_resources(self,
        ...
        self.seq = SequenceGP(load=True)
        np.random.seed(seed)

        x, y, gt = get_experimental_X_y()
        self.full_shape = x.shape[1:]

        x = x.astype(np.float32).reshape([-1, np.prod(x.shape[1:])])
        y = y.astype(np.float32).reshape([-1, 1])
    ...
    def score(self, x):
        ...
        y = tf.numpy_function(self.score_np, [x], tf.float32)
        y.set_shape(x.get_shape()[:1])
        return y
    ...
    def score_np(self, x) -> np.ndarray:
        ...
        return self.seq.predict(np.argmax(x.reshape([
            -1, *self.full_shape]), axis=-1)).astype(np.float32)
